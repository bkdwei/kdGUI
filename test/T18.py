...
def set_cell_value(event):  # 双击进入编辑状态
    ...
    def saveedit():
        ...
        okb.destroy()
    okb = ttk.Button(
        root, text='OK', width=4, command=saveedit)
...
def newrow():
    ...
    ipcode.append('IP')
    treeview.insert('', len(
        name) - 1, values=(name[len(name) - 1], ipcode[len(name) - 1]))
    treeview.update()
...
newb = ttk.Button(root, text='新建联系人',
                  width=20, command=newrow)
newb.place(x=120, y=(len(name) - 1) * 20 + 45)
